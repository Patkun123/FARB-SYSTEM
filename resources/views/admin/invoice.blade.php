    ...
    <script>
        function invoiceApp() {
            return {
                ...
                async init() {
                    const res = await axios.get('{{ route('admin.invoice.clients') }}');
                    this.clients = res.data;

                    // ✅ Fetch next invoice number indicator
                    const nextNum = await axios.get('{{ route('admin.invoice.nextInvoiceNumber') }}');
                ...
                async manualSave() {
                    ...
                    this.invoiceNumber = nextNum.data.next_invoice_number;
                        ...
                        const res = await axios.post('{{ route('admin.invoice.store') }}', payload);
