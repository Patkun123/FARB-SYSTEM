...
document.addEventListener("alpine:init", () => {
    Alpine.data("summaryRecordApp", () => ({
        summaries: [],
        async fetchSummaries() {
            try {
                const response = await fetch("{{ url('/admin/summary-records/api') }}");
        ...
        async deleteSummary(id) {
            ...
            try {
                const response = await fetch(`{{ url('/admin/summary-records/api') }}/${id}`, {
                    method: "DELETE",
                    headers: {
