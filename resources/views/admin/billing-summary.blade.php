                    ...
                    <div class="overflow-x-auto max-w-full">
                        <table class="min-w-max border border-gray-300 text-sm relative">
                            <thead class="bg-gray-100">
                                ...
                                <!-- NEW: Tag row below headers (select per date) -->
                                <tr>
                                    <td class="border px-2 py-1 sticky left-0 bg-gray-100 z-20 text-sm">Date Tag</td>
                                    <template x-for="(day, dIndex) in daysRange()" :key="dIndex">
                                        <td class="border px-2 py-1 text-center">
                                            <select x-model="dayTags[dIndex]"
                                                    @change="onTagChange(dIndex)"
                                        ...
                                        <template x-for="(day, dIndex) in daysRange()" :key="dIndex">
                                            <td class="border px-2 py-1">
                                                <input type="number" step="0.1" min="0"
                                                    ...
                                                    @focus="saveHistory()"
                                                    @input.debounce.150ms="onDailyInput(emp, dIndex)"
...
<script>
    function billingApp() {
        return {
            ...
            dayTags: [],   // dayTags array stores tag per day
            history: [],
            redoStack: [],
            debounceTimer: null,
            ...
            saveHistory() {
                ...
                this.debounceTimer = setTimeout(() => {
                    this.history.push({
                        ...
                        dayTags: JSON.parse(JSON.stringify(this.dayTags))
                    });
                    this.redoStack = [];
                }, 300);
            },
            ...
            // Called when start or end dates change — updates days range and ensures dayTags and employee daily arrays align.
            onDateRangeChange() {
                const oldLen = this.dayTags.length || 0;
                const days = this.daysRange();
                const newLen = days.length;

                if (!this.dayTags) this.dayTags = [];
                if (this.dayTags.length < newLen) {
                    for (let k = this.dayTags.length; k < newLen; k++)
                        this.dayTags.push('regular');
                } else if (this.dayTags.length > newLen) {
            ...
            daysRange() {
                ...
                if (!this.dayTags) this.dayTags = [];
                if (this.dayTags.length < days.length) {
                    for (let k = this.dayTags.length; k < days.length; k++)
                        this.dayTags.push('regular');
                } else if (this.dayTags.length > days.length) {
                    this.dayTags.splice(days.length);
                }

                this.employees.forEach(emp => {
                    if (!emp.daily) emp.daily = [];
                    if (emp.daily.length < days.length) {
                        for (let k = emp.daily.length; k < days.length; k++)
                            emp.daily.push(8);
            ...
            computeFromDailyForEmployee(emp) {
                ...
                if (!emp.daily || !this.dayTags) return;

                for (let d = 0; d < emp.daily.length; d++) {
                    let hours = Number(emp.daily[d]) || 0;
                    if (hours < 0) hours = 0;
                    const tag = this.dayTags[d] || 'regular';

                    switch (tag) {
                        case 'regular': emp.reg_hr += hours; break;
                        case 'ot': emp.ot += hours; break;
                        case 'np': emp.np += hours; break;
                        case 'hpnp': emp.hpnp += hours; break;
                        case 'holiday': emp.reg_hol += hours; break;
                        case 'special_holiday': emp.spec_hol += hours; break;
                        case 'no_work': break;
                        default: emp.reg_hr += hours;
                    }
                }

                emp.reg_hr = Math.round((emp.reg_hr || 0) * 10) / 10;
                emp.ot = Math.round((emp.ot || 0) * 10) / 10;
                emp.np = Math.round((emp.np || 0) * 10) / 10;
                emp.hpnp = Math.round((emp.hpnp || 0) * 10) / 10;
                emp.reg_hol = Math.round((emp.reg_hol || 0) * 10) / 10;
            ...
            totalPay(emp) {
                ...
                const getRate = (field) =>
                    emp.useCustom ? (c[field] != null ? c[field] : r[field]) : r[field];
            ...
            categoryTotalPay(field) {
                return this.employees.reduce((sum, emp) => {
                    ...
                    const rate = emp.useCustom
                        ? (emp.customRates[field] != null ? emp.customRates[field] : this.rates[field])
                        : this.rates[field];
            ...
            addEmployee() {
                ...
                const dailyInit = new Array(daysCount).fill(8);
